...
function ProductGallery({ images }) {
  ...
  return (
    <div>
      {/* Main image */}
      <div style={{ position:"relative", background:"#f2f2f0", overflow:"hidden", borderRadius:"8px 8px 0 0" }}>
        <img
          src={images[idx]}
          style={{ width:"100%", height:260, objectFit:"contain", objectPosition:"center", display:"block", padding:"16px" }}
          alt=""
        />
        {images.length > 1 && idx > 0 && (
          <button onClick={() => setIdx(idx-1)} style={{ position:"absolute", left:10, top:"50%", transform:"translateY(-50%)", background:"rgba(255,255,255,0.95)", border:`1px solid #e8e6e2`, color:"#1a1a1a", width:32, height:32, borderRadius:"50%", fontSize:18, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 1px 4px rgba(0,0,0,0.1)", lineHeight:1 }}>‹</button>
        )}
        {images.length > 1 && idx < images.length-1 && (
          <button onClick={() => setIdx(idx+1)} style={{ position:"absolute", right:10, top:"50%", transform:"translateY(-50%)", background:"rgba(255,255,255,0.95)", border:`1px solid #e8e6e2`, color:"#1a1a1a", width:32, height:32, borderRadius:"50%", fontSize:18, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 1px 4px rgba(0,0,0,0.1)", lineHeight:1 }}>›</button>
        )}
      </div>
      {/* Thumbnails */}
      {images.length > 1 && (
        <div style={{ display:"flex", gap:6, padding:"8px 10px", background:"#f2f2f0", borderTop:`1px solid #e8e6e2` }}>
          {images.map((img, i) => (
            <button key={i} onClick={() => setIdx(i)} style={{
              width:60, height:60, borderRadius:6, border: i===idx ? `2px solid #00a896` : `2px solid #e8e6e2`,
              padding:3, cursor:"pointer", overflow:"hidden", background:"#fff",
              opacity: i===idx ? 1 : 0.55, transition:"all 0.15s", flexShrink:0,
            }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:100, background:"rgba(250,250,248,0.95)", backdropFilter:"blur(12px)", borderBottom:`1px solid ${col.line}` }}>
        <div style={{ maxWidth:520, margin:"0 auto", padding:"14px 20px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            <img src="/img/psc_logo_2025.png" alt="Pharma Service" style={{ height:28, width:"auto", objectFit:"contain" }} />
