...
export default function Home() {
  ...
  return (
    <div style={{ fontFamily:"'Arsenal',sans-serif", color:C.cream, background:C.navy }}>
      ...
      {/* PARTNER TERMS STRIP */}
      <div style={{ background:C.teal, padding:"40px 80px", display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:2 }}>
        {[
          { v:"25%", l:"Base Discount", sub:"Flat across all 6 products. No exceptions." },
          { v:"120", l:"Day Credit Terms", sub:"Zero upfront. Zero deposit. Zero risk." },
          { v:"6+1", l:"Minimum Bonus Tier", sub:"Free units on every order, every time." },
        ].map((t,i) => (
          <div key={i} style={{ padding:"24px 28px", borderRight: i<2 ? "1px solid rgba(10,22,40,0.15)" : "none" }}>
            <div style={{ fontFamily:"'Agrandir',sans-serif", fontSize:42, fontWeight:700, color:C.navy, lineHeight:1 }}>{t.v}</div>
            <div style={{ fontFamily:"'Agrandir',sans-serif", fontSize:15, fontWeight:700, color:C.navy, marginTop:4, marginBottom:6 }}>{t.l}</div>
            <div style={{ fontSize:13, color:"rgba(10,22,40,0.6)", fontStyle:"italic" }}>{t.sub}</div>
          </div>
        ))}
      </div>
